<!-- prompt: concept | version: 1.3.2 | 2026-09-11 -->
...
**Exposure.** Every scene must read clearly on a phone screen: keep visible mid-tones and at least one lit area of the frame, even in night or dusk scenes. Never author a scene that is mostly black or a single dim light in darkness; the final fade to black is applied by the render on top of the last scene, so the last scene itself should be as legible as the others. Scene durations must add up to the chosen duration and sit inside `format_spec`. Ambient viewers stay for stillness: fewer, longer scenes beat many short ones, whatever the total length. No scene shorter than `format_spec.min_scene_seconds` (90 seconds if it is not given).
